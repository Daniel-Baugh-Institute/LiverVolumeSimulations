...
ddir <- getwd()
pass_to_clst <- list(ls())

no_cores <- detectCores(logical = TRUE)
cl <- makeCluster(no_cores/2)
registerDoParallel(cl)

clusterExport(cl, list("animal_list","cd_vals","ddir","eps","k0_guess","LiverRegenODE","lower","M_vals","Nss","Run_Scan","t_0","t_f","upper"))
clusterEvalQ(cl, expr = {library(knitr);
             library(fOptions);
             library(pracma);
             library(NlcOptim);
             library(openxlsx);
...
clusterEvalQ(cl, setwd(ddir))

res_ults <- parLapply(cl, animal_list, fun = Run_Scan)
