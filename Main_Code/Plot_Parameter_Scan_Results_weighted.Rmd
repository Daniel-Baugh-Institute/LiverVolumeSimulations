...
Read in Parameter Scan data
```{r}
comb_res_ults <- readRDS("KCD_Par_ParScan_LLonly_weighted.RDS")

names(comb_res_ults) <- c("11171", "11172", "11173", "11174", "A11", "A12", "A13", "A14", "A15", "A16", "A33", "A35", "A36", "A41", "A42", "A43", "A44", "A45", "A46", "BCH27", "BCH28", "BCH29", "BCHF30", "BCHF31", "BUS21", "BUS22", "BUS23", "BUS24")

all_dat <- read.csv("Auxilliary_Code_and_data/All_Rat_Data_final_06_01_2021.csv")
...
Convert .RDS file into two tables, MLLs (log-liklihood values associated with a particular M), and CdLLs (log-liklihood values associated with a particular Kcd)
```{r}
MLLs <- matrix(nrow = 100, ncol = 30, dimnames = list(1:100, 1:30))
CdLLs <- matrix(nrow = 100, ncol = 30, dimnames = list(1:100, 1:30))

logRange_r <- exp(seq(log(5e-4), log(0.5), length.out = 100))

test <- comb_res_ults[[1]]
test_cnames <- colnames(test)
trim_cnames <- scientific(logRange_r, digits = 3, scale = 1, trim = TRUE)

real_cnames <- paste("Kcd", trim_cnames, sep = "_")


base_ind <- which(abs(logRange_r - 0.01) == min(abs(logRange_r - 0.01)))


for (i in 1:length(comb_res_ults)) {
  dtemp <- comb_res_ults[[i]]
  if (is.matrix(dtemp)){
    MLLs[,i] <- dtemp[,base_ind]
    colnames(MLLs)[i] <- animal_list[i]
    rownames(MLLs) <- rownames(dtemp)
    CdLLs[,i] <- dtemp["M_20.91",]
    rownames(CdLLs) <- real_cnames
    colnames(CdLLs)[i] <- animal_list[i]
    
    colnames(dtemp) <- real_cnames
    comb_res_ults[[i]] <- dtemp
...
Set up the meta data to help with plotting.
```{r}
annotdf <- unique(all_dat[,c(1,3,4)], fromLast = F)
annotdf$Diet <- factor(annotdf$Diet, levels = c("Chow", "Carbohydrate", "Ethanol"))
rownames(annotdf) <- c()
...
All Log Likelihoods for each animal and M value (Figure 5B)
```{r}
rgw_pal <- colorspace::sequential_hcl(n = 300, h1 = 140, h2 = 80, c1 = 50, cmax = NA, c2 = 10, l1 = 40, l2 = 97, p1 = 0.7, p2 = 1.8, rev = TRUE)


eee <- pheatmap::pheatmap(na.omit(t(MLLs)), cluster_cols = F, 
                   cluster_rows = T, 
                   scale = "none", 
...
cols = annotdf$DietColors[order(match(annotdf$Animal, eee$gtable$grobs[[5]]$label))] 
fots = annotdf$SexFonts[order(match(annotdf$Animal, eee$gtable$grobs[[5]]$label))] 
...
All Log Likelihoods for each animal and Kcd value (Figure 5C)
```{r}
fff <- pheatmap::pheatmap(na.omit(t(CdLLs)), cluster_cols = F, 
                   cluster_rows = T, 
                   scale = "none", 
                   color = rgw_pal,
                   labels_col = c("5e-4","","","", "---------------->", "","","", "0.5"), 
                   angle_col = 0,
                  #show_rownames = F,
                   #legend_breaks = c(2,1,0,-1,2),
                   main = "K Cell Death vs LogLikelihood of Residuals, PHx Liver Regeneration Model")

cols = annotdf$DietColors[order(match(annotdf$Animal, fff$gtable$grobs[[5]]$label))] 
fots = annotdf$SexFonts[order(match(annotdf$Animal, fff$gtable$grobs[[5]]$label))] 
...
```{r}
maxs_M_LL <- matrix(unlist(strsplit( names(unlist(apply(MLLs,2,which.max))), split = "_" )), ncol = ncol(MLLs))
ttt <- unlist(strsplit(as.character(maxs_M_LL[1,]), split = ".M"))
maxs_M_LL[1,] <- ttt
maxs_M_LL <- t(maxs_M_LL)
colnames(maxs_M_LL) <- c("Animal", "M")

maxs_CD_LL <- matrix(unlist(strsplit( names(unlist(apply(CdLLs,2,which.max))), split = "_" )), ncol = ncol(CdLLs))
...
#saveRDS(maxs_all, file = "Best_fit_M_Kcd_all_animals_weighted.RDS")
```

Log-Likelihood for all values of M and Kcd, one heatmap made for each animal. Used to create Figure 5D-G
```{r}
...
for (i in 1:length(comb_res_ults)) {
  ...
  if (is.matrix(dtemp)){
    ...
    pcolors = rgw_pal
    pbreaks = c(seq(from = -20, to = 10, length.out = 300))
    ...
    phm <- heatmap.2(na.omit(dtemp), dendrogram = 'none',
                     scale = "none", trace = 'none', symkey = FALSE, symbreaks = TRUE,
                     col = pcolors, 
                     breaks = pbreaks, 
                     Rowv = F,
...
M_kap_vols <- vol_dat[[1]]
# M_theta_vols <- vol_dat[[2]]
# M_beta_vols <- vol_dat[[3]]

#colnames(M_kap_vols[[1]])


use_these <- M_kap_vols[[2]]
...
rownames(use_these) <- rev(real_cnames)

```


Volume at 1 week parameter scan for each animal, highlighting top 5% best fit data, Figure 6
```{r}
for (i in 1:length(comb_res_ults)) {
  #browser()
  dtemp <- comb_res_ults[[i]]
  if (is.matrix(dtemp)) {
    ...
    #relvolsall <- data.table:merge(x = relvols, y = dtemp, by = c("Kap", "MetL"), all = TRUE)
    
    mtind <- apply(dtemp, 1, function(x){ which(relvols$Kap == x[2] & relvols$MetL == x[1]) })
    ...
    if (i == 1){
      ...
      scale_x_continuous(breaks = c(5,10,15,20,25,30)) +
      scale_y_log10() + theme(axis.text = element_text(size = 20), axis.title = element_text(size = 20))
      print(vxf)
    }
