...
animal_list <- c("11171", "11172", "11173", "11174", "A11", "A12", "A13", "A14", "A15", "A16", "A33", "A35", "A36", "A41", "A42", "A43", "A44", "A45", "A46", "BCH27", "BCH28", "BCH29", "BCHF30", "BCHF31", "BUS21", "BUS22", "BUS23", "BUS24")


#animal_list <- c("BUS21", "BUS22", "BUS23", "BUS24")

Run_Scan <- function(animal_n) {
  #browser()
  print(paste('Trying', as.character(animal_n)))
  relRfile <- paste(animal_n, "_datagather_2021.R", sep = "")
  source(file = as.character(relRfile))
  CombScanLL <- matrix(data = NA, nrow = 100, ncol = 100)
  for (j in 1:length(cd_vals)) {
    ...
    for(i in 1:length(M_vals)){
      ...
      try({
        ...
        weights <- rep(1, length(res_iduals))
        weights[length(res_iduals)] <- 5
        res_iduals_w <- weights * res_iduals
        
        llh <- sum(dnorm(x = res_iduals_w, mean = 0, sd = sd(res_iduals), log = TRUE))
        CombScanLL[i,j] <- llh
      })
    }
  }
  rownames(CombScanLL) <- paste("M_", round(M_vals, digits = 2), sep = "")
  colnames(CombScanLL) <- paste("Kcd_", round(cd_vals, digits = 2), sep = "")
  return(CombScanLL)
}
...
nCors <- detectCores() / 2
res_ults <- mclapply(animal_list, Run_Scan, mc.cores = nCors)

saveRDS(res_ults, file = "KCD_Par_ParScan_LLonly_weighted.RDS")
