...
Th_vals <- seq(from = 4.5e-3, to = 1.5e-2, length.out = 100)
Bet_vals <- seq(from = 1e-3, to = 9e-3, length.out = 100)

Pars_to_scan <- c(30,31,32)
ParamRanges <- list(cd_vals, Th_vals, Bet_vals)

Run_Scan <- function(ParamN) {
  #browser()
  print(paste('Trying Scan of M and k#', ParamN))
  relRfile <- paste("11171", "_datagather_2021.R", sep = "")
  source(file = as.character(relRfile))
  CombScanVols <- vector(mode = "list", length = 2)
  CombScanVols[[1]] <- matrix(data = NA, nrow = 100, ncol = 100)
  CombScanVols[[2]] <- matrix(data = NA, nrow = 100, ncol = 100)
  ParN <- ParamN - 29
  parSeq <- ParamRanges[[ParN]]
  
  for (j in 1:length(parSeq)){
    k0_guess[ParamN] <- parSeq[j]
    for(i in 1:length(M_vals)){
      ...
      Vol1 <- Nf[which.min(abs(tf_1 - 24))]
      Vol2 <- Nf[which.min(abs(tf_1 - 168))]
      CombScanVols[[1]][i,j] <- Vol1
      CombScanVols[[2]][i,j] <- Vol2
    }
  }
  rownames(CombScanVols[[1]]) <- paste("M_", round(M_vals, digits = 2), sep = "")
  rownames(CombScanVols[[2]]) <- paste("M_", round(M_vals, digits = 2), sep = "")
  colnames(CombScanVols[[1]]) <- paste("Par", ParamN, round(parSeq, digits = 5), sep = "")
  colnames(CombScanVols[[2]]) <- paste("Par", ParamN, round(parSeq, digits = 5), sep = "")
  return(CombScanVols)
}


#Test <- Run_Scan(animal_n = animal_list[21])

nCors <- detectCores() / 3
res_ults <- mclapply(Pars_to_scan, Run_Scan, mc.cores = nCors)

saveRDS(res_ults, file = "Vol_Data_.RDS")

# print('Trying 11171')	
# CombScanres_11171 <- list()
# CombScanLL_11171 <- data.frame(NULL)
# source('11171_datagather_2021.R')
# for (j in 1:length(cd_vals)) {
#   k0_guess[30] <- cd_vals[j]
#   for(i in 1:length(M_vals)){
#     k0_guess[1] <- M_vals[i]
#     out_1 <- ode23s(LiverRegenODE, t_0, t_f, x_0, k = as.matrix(k0_guess), hmax = 0.1)
#     xf_1 <- out_1$y
#     tf_1 <- out_1$t
#     Nf = as.matrix(xf_1[,1]+xf_1[,11]*(xf_1[,2]+xf_1[,3]))
#     outdat <- data.frame(t = out_1$t, Vol = Nf)
#     rownames(outdat) <- c()
#     #print(M_vals[i])
#     try({
#       t_ind <- unlist(lapply(data_t, function(x) {which.min(abs(tf_1 - x))}))
#       res_iduals <- Nf[t_ind,] - data_v
#       llh <- sum(dnorm(x = res_iduals, mean = 0, sd = sd(res_iduals), log = TRUE))
#       CombScanLL_11171[i,j] <- llh
#       out_trim <- outdat[t_ind,]
#       out_trim$K_cd <- rep(cd_vals[j], times = nrow(out_trim))
#       out_trim$M <- rep(M_vals[i], times = nrow(out_trim))
#       rownames(out_trim) <- c(); 
#       CombScanres_11171[[i]] <- out_trim
#     })
#   }
# }
# rownames(CombScanLL_11171) <- paste("M_", round(M_vals, digits = 2), sep = "")
# colnames(CombScanLL_11171) <- paste("Kcd_", round(cd_vals, digits = 2), sep = "")
# write.table(CombScanres_11171, sep = "\t", file = "11171_CombScanLL_4_23_2021.text")
# saveRDS(CombScanLL_11171, file = "11171_CombScanres_4_23_2021.RDS")
# print('11171 done')
# rm(list = c("data_t", "data_v", "initialFraction"))
# gc()
